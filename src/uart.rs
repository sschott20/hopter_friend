...
pub fn print_chunk(chunk: &Chunk) {
    eprintln!("Message: {:?}", chunk.message);
    eprintln!("Flags: {:#08b}", chunk.flags);
    eprintln!("Checksum: {}", chunk.checksum);
}
...
impl<'a, T: UartRW> UartCrc<'a, T> {
    ...
    pub fn listen_for_response(&mut self) -> Result<Flags, UartError> {
        ...
        match flags.response_type {
            Some(ResponseType::Ack) => eprintln!("Received Ack"),
            Some(ResponseType::Nack) => eprintln!("Received Nack"),
    ...
    pub fn listen_for_data(&mut self) -> Result<Vec<u8>, UartError> {
        ...
        while current_read_size < data_size {
            ...
            if chunk.flags & 0x0F == last_sequence {
                ...
                eprintln!("Sequence mismatch, requesting retransmission");
                self.send_nack()?;
            } else {
    ...
    pub fn send_data(&mut self, data: Vec<u8>) -> Result<(), UartError> {
